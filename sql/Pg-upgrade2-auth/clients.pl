...
sub _copy_fields_to_data_database {
  ...
  my $dbh = SL::DBConnect->connect('dbi:Pg:dbname=' . $client->{dbname} . ';host=' . $client->{dbhost} . ';port=' . $client->{dbport} . ';sslmode=require;channel_binding=required?options=endpoint%3Dep-young-darkness-a2bspxhq-pooler',
